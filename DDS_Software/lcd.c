...
static inline void _delayFourCycles(unsigned int __count) {
	if (__count == 0) __asm__ __volatile__( "rjmp 1f\n 1:" );
	// 2 cycles
...
static void lcd_write(uint8_t data, uint8_t rs) {
	...
	if ((&LCD_DATA0_PORT == &LCD_DATA1_PORT) && (&LCD_DATA1_PORT == &LCD_DATA2_PORT)
			&& (&LCD_DATA2_PORT == &LCD_DATA3_PORT) && (LCD_DATA0_PIN == 0) && (LCD_DATA1_PIN == 1)
...
static inline void lcd_newline(uint8_t pos) {
	...
	if (pos < LCD_START_LINE3) addressCounter = LCD_START_LINE2;
		...
		if ((pos >= LCD_START_LINE2) && (pos < LCD_START_LINE4)) addressCounter = LCD_START_LINE3;
		else
			if ((pos >= LCD_START_LINE3) && (pos < LCD_START_LINE2)) addressCounter = LCD_START_LINE4;
...
void lcd_gotoxy(uint8_t x, uint8_t y) {
	...
	if (y == 0) lcd_command((1 << LCD_DDRAM) + LCD_START_LINE1 + x);
		...
		if (y == 1) lcd_command((1 << LCD_DDRAM) + LCD_START_LINE2 + x);
			...
			if (y == 2) lcd_command((1 << LCD_DDRAM) + LCD_START_LINE3 + x);
			else /* y==3 */
...
void lcd_init(uint8_t dispAttr) {
	...
	if ((&LCD_DATA0_PORT == &LCD_DATA1_PORT) && (&LCD_DATA1_PORT == &LCD_DATA2_PORT)
			&& (&LCD_DATA2_PORT == &LCD_DATA3_PORT) && (&LCD_RS_PORT == &LCD_DATA0_PORT)
			&& (&LCD_RW_PORT == &LCD_DATA0_PORT) && (&LCD_E_PORT == &LCD_DATA0_PORT)
			&& (LCD_DATA0_PIN == 0) && (LCD_DATA1_PIN == 1) && (LCD_DATA2_PIN == 2)
			&& (LCD_DATA3_PIN == 3) && (LCD_RS_PIN == 4) && (LCD_RW_PIN == 5) && (LCD_E_PIN == 6)) {
